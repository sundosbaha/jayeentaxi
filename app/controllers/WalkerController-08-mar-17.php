        ...
        if ($validator->fails()) {
            ...
            if ($walker_data = $this->getWalkerData($walker_id, $token, $is_admin)) {
                    ...
                    if ($request = Requests::find($request_id)) {
                        if ($request->current_walker == $walker_id) {
                            if ($accepted == 1) {
                                ...
                                if(Config::get('app.default_payment') == 'metropago' && $request->payment_mode == 0)
                                {
                                    $payment=DB::table('payment')->where('owner_id','=',$request->owner_id)->where('is_default','=',1)->first();
                                    $sdk = new  MetropagoGateway(Config::get('app.metropago_mode'), Config::get('app.metropago_merchant_id'), Config::get('app.metropago_terminal_id'), "", "");
                                    ...
                                    $card = new CreditCard();
                                    $transRequest->CustomerData->CustomerId = $payment->customer_id;//Customer Token
                                    $card->Token = $payment->card_token;//Card Token
                                    $card->cvv=$payment->cvv;
                                    $transRequest->CustomerData->CreditCards[] = $card;
                                    $settings = Settings::where('key', 'pre_auth_amt')->first();
                                    $amt=number_format((float)$settings->value,2,'.','');
                                    $transRequest->Amount = $amt;
                                    $transRequest->OrderTrackingNumber="track_no".$request_id;
                                    ...
                                    if ($sale_response->ResponseDetails->IsSuccess)
                                    {
                                        ...
                                        $transRequest->Amount = "558.63";
                                        $refund_response = $TrxManager->Refund($transRequest);
                                        if(!$refund_response->ResponseDetails->IsSuccess)
                                        {
                                            goto fail;
                                        } */
                                    ...
                                    else
                                    {
                                        fail:
                                        // request ended
                                        Requests::where('id', '=', $request_id)->update(array('current_walker' => 0, 'status' => 1, 'owner_reason' => 'card'));
                                        /* $driver = Keywords::where('id', 1)->first(); */
                                        $owne = Owner::where('id', $request->owner_id)->first();
                                        /* $driver_keyword = $driver->keyword; */
                                        $owner_data_id = $owne->id;
                                        send_notifications($walker_id, "walker", 'Payment Card is Invalid', 'Payment Card is Invalid');
                                        send_notifications($owner_data_id, "owner", 'Payment Card is Invalid', 'Payment Card is Invalid');
                                        $response_array = array('success' => false,'error_code' => 470,"error" => "Payment Card is Invalid");
                                        $response_code = 200;
                                        $response = Response::json($response_array, $response_code);
                                        return $response;
                                ...
                                $title = "Conductor aceptar el viaje";
                                ...
                                $subject = "Preparate para tu Viaje";
                                ...
                                if (isset($request_meta->walker_id)) {
                                    ...
                                    $owner_data_id = $owne->id;
                                    send_notifications($owner_data_id, "owner", 'encontrado ningún controlador', 'No ' . $driver_keyword . ' are available right now in your area. Kindly try after sometime.');
                                }
                            }
                            $response_array = array('success' => true);
                            $response_code = 200;
                        ...
                        $response_array = array('success' => false, 'error' => 'Solicitud ID no encontrada', 'error_code' => 405);
                        $response_code = 200;
                    }
                } else {
                ...
                if ($is_admin) {
                    ...
                    $response_array = array('success' => false, 'error' => 'Token Invalido', 'error_code' => 406);
                }
                $response_code = 200;
            }
        }
                        ...
                        if ($request = Requests::find($request_id)) {
                            ...
                            if ($request->confirmed_walker == $walker_id) {
                                ...
                                if ($request->is_started == 1) {
                                    ...
                                    /* CALCULATION OF ADMIN AND DRIVER PERCENTAGE -- Rangasamy */
