            ...
            if ($owner_data = $this->getOwnerData($owner_id, $token, $is_admin)) {
                ...
                if (is_token_active($owner_data->token_expiry) || $is_admin) {
                    ...
                    if ($ledger) {
                        ...
                        $response_array = array('success' => false, 'error' => 'This user does not have a referral code', 'error_code' => 405);
                    }
                ...
                if (is_token_active($owner_data->token_expiry) || $is_admin) {
                    ...
                    foreach ($request_data as $data) {
                        ...
                        if ($setting_zone->value == 1) {
                            ...
                            $zoneTypesRecs = getTypeBesedZoneTyeRecords($data->req_typ);

                            $setbase_distance = $zoneTypesRecs[0]->base_distance;
                            $setbase_price = $zoneTypesRecs[0]->base_price;
                            $setdistance_price = $zoneTypesRecs[0]->price_per_unit_distance;
                            $settime_price = $zoneTypesRecs[0]->price_per_unit_time;
                ...
                if (is_token_active($owner_data->token_expiry) || $is_admin) {
                    if (Input::get('new_password')) {
                        if (Input::get('old_password') != "") {
                            ...
                            $response_array = array('success' => false, 'error' => 'Old Password must not be blank', 'error_code' => 625);
                            $response_code = 200;
                        }
                    } else {
                        // Do necessary operations
