    ...
    public function register() {
        ...
        if ($validator->fails()) {
            ...
            if (Owner::where('email', '=', $email)->first()) {
                $response_array = array('success' => false, 'error' => 'Email ID already registered', 'error_code' => 402);
                $response_code = 200;
            } else {
				...
				{
					$response_array = array('success' => false, 'error' => 'Phone number already registered', 'error_code' => 402);
                $response_code = 200;
				 goto response;
				}
				...
				sms_notification($owner->id, 'owner', $message);
					$response_array = array(
                    'success' => true,
                    'id' => $owner->id,
                    'first_name' => $owner->first_name,
        ...
        if (Input::has('email') && Input::has('password')) {
            ...
            if ($validator->fails()) {
                ...
                if ($owner = Owner::where('email', '=', $email)->orWhere('phone', 'LIKE', '%'.$email.'%')->first()) {
					if(!$owner->is_active)
					{
						 $response_array = array('success' => false, 'error' => 'User Inactive', 'error_code' => 401);
                $response_code = 200;
				goto response;
					}
	...
	public function checkotp()
	{
		...
		if($owner)
		{
            ...
            ->update(array('is_active' => 1));
				...
				{
					$response_array = array('success' => false, 'error' => 'OTP code time exceeds', 'error_code' => 410); //otp time exceeds
				}
			}
			else
			{
				$response_array = array('success' => false, 'error' => 'OTP code is incorrect', 'error_code' => 410);  //otp wrong
			}
		}
		else
		{
			$response_array = array('success' => false, 'error' => 'Owner ID not Found', 'error_code' => 410);  //owner not found
		}
 $response_code = 200;
  $response = Response::json($response_array, $response_code);
        return $response;
	...
	public function deleteuser()
	{
		...
		{
		$response_array = array('success' => false, 'error' => "User doesn't Exit", 'error_code' => 410);	
		}
		 $response_code = 200;
  $response = Response::json($response_array, $response_code);
        return $response;
	...
	public function otpresend()
	{
		...
		DB::table('owner')
		 ...
		 $settings = Settings::where('key', 'sms_otp')->first();
                $admin_message = $settings->value;
				$message=str_replace('%number%',$otp_number,$admin_message);
					sms_notification($ownerid, 'owner', $message);
