                ...
                if (is_token_active($walker_data->token_expiry)) {
                    ...
                    if ($request = Requests::find($request_id)) {
                        if ($request->confirmed_walker == $walker_id) {
                            ...
                            $response_array = array('success' => true, 'is_available' => $walker_data->is_active, 'is_approved' => $walker_data->is_approved, 'is_approved_txt' => $txt_approve, 'request' => $request_data, 'bill' => $bill ? $bill : null);
                            $response_code = 200;
                        } else {
                            /* $driver = Keywords::where('id', 1)->first();
                                ...
                                if ($request->is_started == 1) {
                                    ...
                                    if ($setting_zone->value == 1) {
                                        ...
                                        if (!empty($zoneRecords)) {
                                            //Find Zone Division for latitude and longitude value based
                                            $zone_id = (!empty($walker_data->zone_id) ? $walker_data->zone_id : '');
                                        }
