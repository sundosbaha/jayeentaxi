...
class WalkerController extends BaseController {
    ...
    public function cancellation() {
        ...
        if ($validator->fails()) {
            ...
            if ($owner_data = $this->getWalkerData($owner_id, $token, $is_admin)) {
                ...
                if (is_token_active($owner_data->token_expiry) || $is_admin) {
                    ...
                    if($request_id != -1) {
                        ...
                        $response_array = array('success' => false, 'error' => 'Request Id Not Found', 'error_code' => 628);
                        $response_code = 200;
                    }
    ...
    public function register() {
        ...
        $airport_badge = 0;
        if(Input::get('airport_badge')) {
            $airport_badge = Input::get('airport_badge');
        }
        $car_number = 0;
        if (Input::has('car_number')) {
            $car_number = Input::get('car_number');
        }
        $proof_commercial_insurance = Input::file('proof_commercial_insurance');
        $proof_commercial_license_front=Input::file('proof_commercial_lfront');
        $proof_commercial_license_back=Input::file('proof_commercial_lback');
        ...
        if ($password != "" and $social_unique_id == "") {
            $validator = Validator::make(
                        ...
                        'login_by' => $login_by,
                         ...
                         'car_model' => $car_model,
                               ...
                               /* 'business_name' => $business_name,
                                ...
                                'business_insurance' => $proof_commercial_insurance,
                                'business_license_front'=>$proof_commercial_license_front,
                                'business_license_back'=>$proof_commercial_license_back, */
                    ...
                    'business_insurance' =>'required|mimes:jpeg,bmp,png',
                    'business_license_front'=> 'required|mimes:jpeg,bmp,png',
                    'business_license_back'=> 'mimes:jpeg,bmp,png', */
                            )
            );
        ...
        if ($validator->fails()) {
            ...
            if (Walker::where('email', '=', $email)->first()) {
                $response_array = array('success' => false, 'error' => 'Email ID already Registred', 'error_code' => 602);
                $response_code = 200;
            } else {
                ...
                //$walker->home_address = $home_address;
                $walker->email = $email;
                $walker->phone = $phone;
                ...
                $s4_url = "";
                if (Input::hasfile('proof_commercial_insurance')) {
                    $ext = Input::file('proof_commercial_insurance')->getClientOriginalExtension();
                    Input::file('proof_commercial_insurance')->move(public_path() . "/uploads", $file_name . "." . $ext);
                    $local_url = $file_name . "." . $ext;
                    ...
                    if (Config::get('app.s3_bucket') != "") {
                        ...
                        $s4_url = $s3->getObjectUrl(Config::get('app.s3_bucket'), $file_name);
                    } else {
                        $s4_url = asset_url() . '/uploads/' . $local_url;
                    }
                }

                $s5_url = "";
                if (Input::hasfile('proof_commercial_lfront')) {
                    $ext = Input::file('proof_commercial_lfront')->getClientOriginalExtension();
                    Input::file('proof_commercial_lfront')->move(public_path() . "/uploads", $file_name . "." . $ext);
                    $local_url = $file_name . "." . $ext;
                    ...
                    if (Config::get('app.s3_bucket') != "") {
                        ...
                        $s5_url = $s3->getObjectUrl(Config::get('app.s3_bucket'), $file_name);
                    } else {
                        $s5_url = asset_url() . '/uploads/' . $local_url;
                    }
                }

                $s6_url = "";
                if (Input::hasfile('proof_commercial_lback')) {
                    $ext = Input::file('proof_commercial_lback')->getClientOriginalExtension();
                    Input::file('proof_commercial_lback')->move(public_path() . "/uploads", $file_name . "." . $ext);
                    $local_url = $file_name . "." . $ext;
                    ...
                    if (Config::get('app.s3_bucket') != "") {
                        ...
                        $s6_url = $s3->getObjectUrl(Config::get('app.s3_bucket'), $file_name);
                    } else {
                        $s6_url = asset_url() . '/uploads/' . $local_url;
                    }
                }

                /*new things*/
                $walker->license_front= $s5_url;
                $walker->license_back= $s6_url;
                //$walker->business_name= ( !empty( $business_name ) ? $business_name : '');
                //$walker->business_address= ( !empty( $business_address ) ? $business_address : '');
                $walker->city = $city;
                ...
                $walker->type = $type;
                $walker->car_make = $car_make;
                $walker->car_model = $car_model;
                $walker->car_number = $car_number;
                $walker->airport_badge = $airport_badge;
                $walker->proof_commercial_insurance = $s4_url;
                if ($social_unique_id != "") {
                ...
                $response_array = array(
                    'success' => true,
                    'driver' => array('id' => $walker->id,
                    'first_name' => $walker->first_name,
                    'last_name' => $walker->last_name,
                    //'home_address' => $walker->home_address,
                    'phone' => $walker->phone,
                    'email' => $walker->email,
                    ...
                    'car_make' => $walker->car_make,
                    'car_model' => $walker->car_model,
                    'car_number' => $walker->car_number,
                    'airport_badge' => (!empty( $walker->airport_badge == 1) ? true : false),
                    'proof_commercial_insurance' => $walker->proof_commercial_insurance,
                    /* 'type' => $myType, */
                    'type' => $walker->type,
                    'is_approved' => $walker->is_approved ? true : false,
                    'is_approved_txt' => $txt_approve,
                    'is_available' => $walker->is_active ? true : false,)
                );
                $response_code = 200;
            }
        }
        ...
        if (Request::isMethod('post')) {
            ...
            if ($validator->fails()) {
                ...
                if ($walker_data = $this->getWalkerData($walker_id, $token, $is_admin)) {
                    ...
                    if (is_token_active($walker_data->token_expiry) || $is_admin) {
                        ...
                        if ($request = Requests::find($request_id)) {
                            if ($request->confirmed_walker == $walker_id) {
                                ...
                                if ($request->is_dog_rated == 0) {
                                    ...
                                    if ($rating) {
                                        if ($owner = Owner::find($request->owner_id)) {
                                           ...
                                           // $new_rate_counter = ($owner->rate_/provider/ratingcount + 1);
                                ...
                                $response_array = array('success' => false, 'error' => 'Service ID doesnot matches with ' . Config::get('app.generic_keywords.Provider') . ' ID', 'error_code' => 627);
                                $response_code = 200;
                            }
                        } else {
                            $response_array = array('success' => false, 'error' => 'Service ID Not Found', 'error_code' => 628);
                            $response_code = 200;
                        }
                    } else {
                        $response_array = array('success' => false, 'error' => 'Token Expired', 'error_code' => 620);
                    ...
                    if ($is_admin) {
                        /* $response_array = array('success' => false, 'error' => '' . $var->keyword . ' ID not Found', 'error_code' => 410); */
                        $response_array = array('success' => false, 'error' => '' . Config::get('app.generic_keywords.Provider') . ' ID not Found', 'error_code' => 621);
                    } else {
                        $response_array = array('success' => false, 'error' => 'Not a valid token', 'error_code' => 622);
                    }
                    $response_code = 200;
        ...
        if ($validator->fails()) {
            ...
            if ($walker_data = $this->getWalkerData($walker_id, $token, $is_admin)) {
                ...
                if (is_token_active($walker_data->token_expiry)) {
                    ...
                    /*Current Walker dateTime update */
                    $queryUpdate = "UPDATE walker SET updated_at=NOW() WHERE id = $walker_id";
                    DB::update(DB::raw($queryUpdate));

                    $query = "SELECT id, later, D_latitude, D_longitude,latitude,longitude,gmap_image, payment_mode, request_start_time ,pickupDetails,dropoffDetails, owner_id,TIMESTAMPDIFF(SECOND,updated_at, '$time') as diff from request where is_cancelled = 0 and status = 0 and current_walker=$walker_id and TIMESTAMPDIFF(SECOND,updated_at, '$time') <= $timeout";
                    ...
                    foreach ($requests as $request) {
                        ...
                        //$data['pickup_address'] =  ( !empty( $request->pickupDetails ) ? $request->pickupDetails : '' );
                       // $data['drop_address'] = ( !empty( $request->dropoffDetails ) ? $request->dropoffDetails : '' );
                       // $data['gmap_url'] = ( !empty( $request->gmap_image ) ? $request->gmap_image : '' );

                        if( !empty( $request->latitude ) && !empty( $request->longitude ) && !empty( $request->D_latitude ) && !empty( $request->D_longitude ) ) {
                            ...
                            $data['estimation'] = ( !empty( $estimation_total ) ? $estimation_total : 0 );
                        }
                        ...
                        $request_data = array();
                        $estimation = array();
                        $request_data['estimation']['estimation'] = ( !empty( $data['estimation'] ) ? $data['estimation'] : 0 );
                        $request_data['estimation']['pickup_address'] = ( !empty( $request->pickupDetails ) ? $request->pickupDetails : '' );
                        $request_data['estimation']['drop_address'] = ( !empty( $request->dropoffDetails ) ? $request->dropoffDetails : '' );
                        $request_data['estimation']['gmap_url'] = ( !empty( $request->gmap_image ) ? $request->gmap_image : '' );
                    ...
                    if ($request = Requests::find($request_id)) {
                        if ($request->current_walker == $walker_id) {
                            if ($accepted == 1) {
                                ...
                                // Send SMS
                                $owner = Owner::find($request->owner_id);
                                $src_address = get_address_trip($owner->latitude, $owner->longitude);
                                ...
                                if (isset($request_meta->walker_id)) {
                                    ...
                                    $msg_array['estimation']['estimation'] = ( !empty( $estimation_total ) ? $estimation_total : 0 );
                                    $msg_array['estimation']['pickup_address'] = $request->pickupDetails;
                                    $msg_array['estimation']['drop_address'] = $request->dropoffDetails;
                                    $msg_array['estimation']['gmap_url'] = ( !empty( $request->gmap_image ) ? $request->gmap_image : '' );
                            ...
                            //DB::table('walker_cancel')->insert(['request_id' => $request_id, 'walker_id' => $walker_id, 'reason' => $reason]);
                            $response_array = array('success' => true);
                            $response_code = 200;
                        } else {
                            ...
                            $response_array = array('success' => false, 'error' => 'Request ID does not matches' . Config::get('app.generic_keywords.Provider') . ' ID', 'error_code' => 627);
                            $response_code = 200;
                        }
                    } else {
                ...
                if (is_token_active($walker_data->token_expiry)) {
                    ...
                    if ($request = Requests::find($request_id)) {
                        if ($request->confirmed_walker == $walker_id) {
                            ...
                            $request_data['request_type'] = ( !empty( $request->request_type ) ? $request->request_type : '');
...
//                            exit;
                            ...
                            if ($request->is_completed == 1) {
                                ...
                                //$bill['one_minute'] = ( !empty($request_typ->waiting_price) ? $request_typ->waiting_price : 0);
                            ...
                            $request_data['bill'] = $bill ? $bill : null;
                            ...
                            $charge['price_per_unit_time'] = $charge['distance_price'] = $charge['base_price'] = $charge['base_distance'] = $floatValue;
                            ...
                            if($request->is_completed == 1 ) {
                                ...
                                if ($setting_zone->value == 1) {
                                    ...
                                    $chagre['base_distance'] = currency_converted($zoneTypesRecs[0]->base_distance);
                                    $chagre['base_price'] = currency_converted($zoneTypesRecs[0]->base_price);
                                    $chagre['distance_price'] = currency_converted($zoneTypesRecs[0]->price_per_unit_distance);
                                    $chagre['price_per_unit_time'] = currency_converted($zoneTypesRecs[0]->price_per_unit_time);
                            ...
                            $request_data['charge_details'] = (!empty( $chagre ) ? $chagre : $charge);



                            $response_array = array('success' => true, 'is_available' => $walker_data->is_active, 'is_approved' => $walker_data->is_approved, 'is_approved_txt' => $txt_approve, 'request' => $request_data);
                            $response_code = 200;
                        } else {
                            /* $driver = Keywords::where('id', 1)->first();
                              $response_array = array('success' => false, 'error' => 'Service ID doesnot matches with ' . $driver->keyword . ' ID', 'error_code' => 407); */
                            $response_array = array('success' => false, 'error' => 'Service ID doesnot matches with ' . Config::get('app.generic_keywords.Provider') . ' ID', 'is_available' => $walker_data->is_active, 'is_approved' => $walker_data->is_approved, 'is_approved_txt' => $txt_approve, 'error_code' => 627);
                            $response_code = 200;
                        }
                    } else {
                        $response_array = array('success' => false, 'error' => 'Service ID Not Found', 'is_available' => $walker_data->is_active, 'is_approved' => $walker_data->is_approved, 'is_approved_txt' => $txt_approve, 'error_code' => 628);
                        $response_code = 200;
                    }
                } else {
                    $response_array = array('success' => false, 'error' => 'Token Expired', 'error_code' => 620);
                            ...
                            if ($request->confirmed_walker == $walker_id) {
                                ...
                                if ($request->confirmed_walker != 0) {
                                  ...
                                  // $title = '' . Config::get('app.generic_keywords.Provider') . ' has started moving towards you';
									//$title = 'conductor ha comenzado a moverse hacia usted';
                                    ...
                                    $response_array = array('success' => false, 'error' => '' . Config::get('app.generic_keywords.Provider') . ' not yet confirmed', 'error_code' => 629);
                                    $response_code = 200;
                                }
                            } else {
                                /* $driver = Keywords::where('id', 1)->first();
                        ...
                        if ($request = Requests::find($request_id)) {
                            if ($request->confirmed_walker == $walker_id) {
                                ...
                                if ($request->is_walker_started == 1) {
                                    ...
                                    $title = 'Driver has arrived at your place';
                              //      $title = 'O motorista chegou na sua localização';
                                    ...
                                    $response_array = array('success' => false, 'error' => 'Service not yet started', 'error_code' => 643);
                                    $response_code = 200;
                                }
                            } else {
                                /* $response_array = array('success' => false, 'error' => 'Service ID doesnot matches with ' . $driver->keyword . ' ID', 'error_code' => 407); */
                        ...
                        if ($request = Requests::find($request_id)) {
                            if ($request->confirmed_walker == $walker_id) {
                                ...
                                if ($request->is_walker_arrived == 1) {
                                    ...
                                    $service_cost['type']=$zone_type->type;
                                    $service_cost['base_price']=$zone_type->base_price;
                                    $service_cost['base_distance']=$zone_type->base_distance;
                                    $service_cost['price_per_unit_distance']=$zone_type->price_per_unit_distance;
                                    $service_cost['price_per_unit_time']=$zone_type->price_per_unit_time;
                                    $service_cost['waiting_price']= $walker_type->waiting_price;
                                    $service_cost['max_size']=$zone_type->max_size;
                                    ...
                                    $settings = Settings::where('key', 'driver_tips')->first();
                                    $driverTips = $settings->value;

                                    $response_array = array(
                                        'success' => true,
                                        'tips_status' => (!empty( $driverTips == 1) ? true : false),
                                        'request_id' => $request_id,
                                        'status' => $request->status,
                                        'confirmed_walker' => $request->confirmed_walker,
                                        'is_walker_started' => $request->is_walker_started,
                                  ...
                                  // $title = 'Sua viagem foi iniciada';
									//$title = 'su viaje se ha iniciado';
                                    $message = $response_array;
                                    ...
                                    $response_array = array('success' => true,'tips_status' => (!empty( $driverTips == 1) ? true : false),'service_cost'=>$service_cost);
                                    $response_code = 200;
                                } else {
                                    /* $response_array = array('success' => false, 'error' => '' . $var->keyword . ' not yet arrived', 'error_code' => 413); */
                                    $response_array = array('success' => false, 'error' => '' . Config::get('app.generic_keywords.Provider') . ' not yet arrived', 'error_code' => 644);
                                    $response_code = 200;
                                }
                            } else {
                                /* $response_array = array('success' => false, 'error' => 'Service ID doesnot matches with ' . $var->keyword . ' ID', 'error_code' => 407); */
    ...
    public function request_walk_completed() {
        ...
        if (Request::isMethod('post')) {
            ...
            if (Input::has('trip_waiting_time')) {
                $trip_waiting_time = Input::get('trip_waiting_time');
            }
            if (Input::has('total_waiting_time')) {
                $total_waiting_time = Input::get('total_waiting_time');
            }
                            ...
                            if ($request->confirmed_walker == $walker_id) {
                                ...
                                if ($request->is_started == 1) {
                                    ...
                                    if ($setting_zone->value == 1) {
                                        ...
                                        $zone_id = isCheckAllZonesTypesFindZoneId($longitude,$latitude);
        ...
        /*print_r($zone_id);
                                        ...
                                        if (!empty($zone_id)) {
                                            ...
                                            $reqServices = RequestServices::where('request_id', $request_id)->get();
                                            //walker Trip completed total calculate values for update Request Service Table
...
die("t");*/
                                            ...
                                            //Trip completed total calculate for Request Service Table values
                                            $total = walkerTripCompletedTotCalculation($request_id);

                                        } else {

                                            //walker Trip completed total calculate values for update Request Service Table
                                            $walkerRecords = walkerTripCompletedUpdateRecords($reqServices, $request->zone_id, $distance, $time,$trip_waiting_time);
                                        ...
                                        foreach ($reqserv as $rse) {
                                            ...
                                            if ($is_multiple_service->value == 0) {
                                                ...
                                                if ($pt->price_per_unit_time == 0) {
                                                    ...
                                                    if($time>$trip_waiting_time)
                                                    {
                                                        $price_per_unit_time = $providertype->price_per_unit_time * ($time-$trip_waiting_time);
                                                    }
                                                    else
                                                    {
                                                        $price_per_unit_time=0;
                                    ...
                                    //Driver Tips store Records in database
                                   /* if(!empty($tips)) {
                                        ...
                                        $total = driverTipsAddCalculation($tips,$total);
                                    }

                                    //Driver Waiting Minutes store Records in database for Request Service
                                    if(!empty($minutes)) {

                                        $waitingPrice = $providertype->waiting_price * $minutes;
                                        $waitingChargePayment = RequestServices::where('request_id', '=',  $request->id)->first();
                                        $waitingChargePayment->waiting_cost = $waitingPrice;
                                        $waitingChargePayment->save();

                                        $total = driverWaitingMinutesAddCalculation($waitingPrice,$total);

                                    }

                                    //Service Tax - Percentage Calculation for total amt
                                    $total = isCheckRequestServiceTaxCalculation($total, 'service_tax'); */

                                    $request->total = $total;
                                    if(!empty($trip_waiting_time) && !empty($total_waiting_time)) {
                                        $tripWaitingTime = isCheckDateDifferenceReturnWaitingMinutes($request->request_start_time, $trip_waiting_time);
                                        $waitingPrice = $providertype->waiting_price * $total_waiting_time;
                                        $waitingChargePayment = RequestServices::where('request_id', '=',  $request->id)->first();
                                        $waitingChargePayment->waiting_cost = (!empty( $waitingPrice ) ? $waitingPrice : 0);
                                        $waitingChargePayment->total_waiting_time = $totalWaitingTime = (!empty( $total_waiting_time ) ? $total_waiting_time : 0);
                                        $waitingChargePayment->waiting_time = $waitingTime = (!empty( $tripWaitingTime ) ? $tripWaitingTime : 0);
                                        $waitingChargePayment->save();
                                        $request->total = $total + ( !empty( $waitingPrice ) ? $waitingPrice : 0 );
                                    }
                                    ...
                                    if(!empty($adminPercentage) && $adminPercentage != 0) {
                                        $percentageRate = "100";
                                        $driverPercentage = $percentageRate - $adminPercentage;

                                        $calcAdminPerPayment = $request['total'] * $adminPercentage / $percentageRate;
                                        $calcDriverPerPayment = $request['total'] * $driverPercentage / $percentageRate;
                                        $walletEntryCheck=DB::table('walker_wallet_state')->where('walker_id',$request->id)->first();
                                        if(!$walletEntryCheck)
                                        {
                                            $walker_history= new WalkerWalletHistory();
                                            $walker_history->walker_history_id = SettlementUniqueIdGeneration();
                                            $walker_history->request_id = $request->id;
                                            $walker_history->walker_id = $request->confirmed_walker;
                                            $walker_history->amount = $calcDriverPerPayment;
                                            $walker_history->save();

                                            $wallet_status=WalkerWalletState::where('walker_id',$request->confirmed_walker)->first();
                                            if($wallet_status)
                                            {
                                                $wallet_status->total_amount += $calcDriverPerPayment;
                                                $wallet_status->total_balance += $calcDriverPerPayment;
                                                $wallet_status->save();

                                            }
                                            else
                                            {
                                                $wallet_status= new WalkerWalletState();
                                                $wallet_status->walker_id=$request->confirmed_walker;
                                                $wallet_status->total_amount = $calcDriverPerPayment;
                                                $wallet_status->total_balance = $calcDriverPerPayment;
                                                $wallet_status->save();
                                            }
                                    ...
                                    if ($request->payment_mode == 1 and $allow_cod == 1) {
                                        ...
                                        Log::info('paypal payment');*/
                                            ...
                                            $user_wallet_state=WalletStatus::where('user_id',$request->owner_id)->first();
                                            if($user_wallet_state->total_balance >= $total)
                                            {
                                                $payment_type = 'Payment By wallet';
                                                $user_wallet_state->total_balance -= $total;
                                                $user_wallet_state->total_spend += $total;
                                                $user_wallet_state->save();

                                                $user_spend_add=new WalletAddSpend();
                                                $user_spend_add->wallet_spend_id=generate_wallet_id('user');
                                                $user_spend_add->amount=$total;
                                                $user_spend_add->user_id=$request->owner_id;
                                                $user_spend_add->request_id=$request->id;
                                                $user_spend_add->save();
                                                ...
                                                $request->is_paid = 0;
                                                $payment_type = "Insuffient Balance In Wallet";
                                                $ledger = Ledger:: where('owner_id', $request->owner_id)->first();
                                                if ($ledger) {
                                                    $ledger_temp = Ledger::find($ledger->id);
                                                ...
                                                /* Client Side Push */
                                                //$title = 'Insuffisível Saldo na carteira Por favor, pague em dinheiro';
                                                $title="Insuffient Balance In Wallet Please Pay By cash";
                                                $response_array = array(
                                                ...
                                                /* Driver Side Push */
                                                $title = 'Please collect cash from client to this trip';
                                                //$title = 'Por favor, receba em dinheiro do passageiro';
                                                $response_array = array('success' => true,
                                                    'message' => $title,
                                                );
                                                $message = $response_array;
                                        ...
                                        if ($total == 0) {
                                            ...
                                            if ($payment_data) {
                                                ...
                                                if (Config::get('app.default_payment') == 'stripe') {
                                                    ...
                                                    try {
                                                        ...
                                                        if ($charge->paid) {
                                                            ...
                                                            //$title = 'Seu cartão é recusado, por favor, pague em dinheiro ao motorista para sua viagem';
                                                            $response_array = array(
                                                                'success' => true, 'message' => $title,);
                                                            $message = $response_array;
                                                            send_notifications($request->owner_id, "owner", $title, $message);
                                                    ...
                                                    try {
                                                        ...
                                                        if ($result->success) {
                                                          ...
                                                          $title = 'Please collect cash from ' . Config::get('app.generic_keywords.User') . ' for your ' . Config::get('app.generic_keywords.Trip') . '.';
                                                          //$title = 'Por favor, receba em dinheiro do passageiro';
															//$title = 'Por favor, recoger dinero en efectivo a partir de usuario para su viaje';
                                                            $response_array = array('success' => true,
                                                                'message' => $title,
                                                            );
                                                            $message = $response_array;
                                                ...
                                                /* Service Fee ( CARD ) - CALCULATION OF ADMIN AND DRIVER PERCENTAGE -- Rangasamy */
                                                $settings = Settings::where('key', 'service_fee_card')->first();
                                                $adminPercentage = (!empty( $settings->value ) ? $settings->value : '');

                                                if(!empty($adminPercentage) && $adminPercentage != 0) {
                                                    $percentageRate = "100";
                                                    $driverPercentage = $percentageRate - $adminPercentage;
                                                    $calcAdminPerPayment = $total * $adminPercentage / $percentageRate;
                                                    $calcDriverPerPayment = $total * $driverPercentage / $percentageRate;
                                                }
                                    ...
                                    $request->driver_per_payment = $calcDriverPerPayment;

                                    if($request->request_type == "Hand") {
                                        $request->is_dog_rated = 1;
...
//die("p");
                                      ...
                                      //  $bill['total'] = currency_converted($request->total - $request->ledger_payment - $request->promo_payment);
                                        ...
                                        $bill['payment_type'] = $request->payment_mode;
                                        $bill['is_paid'] = $request->is_paid;
                                        $bill['tips'] = (!empty($tips) ? $tips : 0);
                                        $bill['waiting_price'] = (!empty($waitingPrice) ? $waitingPrice : 0);
                                        $bill['waiting_time'] = (!empty($waitingTime) ? $waitingTime : 0);
                                        $bill['total_waiting_time'] = (!empty($totalWaitingTime) ? $totalWaitingTime : 0);
                                        $bill['service_tax'] = (!empty($requestserv->service_tax) ? $requestserv->service_tax : 0);
                                        $zoneTypeChargeRecs = ZoneType::where('zone_id', $zone_id)->where('type', $requestserv->type)->first();
                                        $bill['distance_price'] = ( !empty( $zoneTypeChargeRecs->price_per_unit_distance ) ? $zoneTypeChargeRecs->price_per_unit_distance : 0);
                                        $bill['price_per_unit_time'] = ( !empty( $zoneTypeChargeRecs->price_per_unit_time ) ? $zoneTypeChargeRecs->price_per_unit_time : 0 );
                                    }
                                   ...
                                   /* foreach ($rservc as $typ) {
                                    ...
                                    if(!empty($adminPercentage) && $adminPercentage != 0) {
                                        $chagre['admin_per_payment'] = $calcAdminPerPayment;  //Assign Percantage calcuation of admin
                                        $chagre['driver_per_payment'] = $calcDriverPerPayment; //Assign Percantage calcuation of driver
                                    }
                                  ...
                                  /*  print_r($requestserv);
                                    ...
                                    if ($setting_zone->value == 1) {
                                        ...
                                        $chargeType = $requestserv->type;
                                        $zoneTypeChargeRecords = ZoneType::where('zone_id', $zone_id)->where('type', $chargeType)->first();

                                        $chagre['base_price'] = $zoneTypeChargeRecords->base_price;
                                        $chagre['distance_price'] = $zoneTypeChargeRecords->price_per_unit_distance;
                                        $chagre['price_per_unit_time'] = $zoneTypeChargeRecords->price_per_unit_time;
									...
									//$title = 'su viaje se ha completado';
                                  //  $title='Sua viagem foi finalizada';
                                    $message = $response_array;
                        ...
                        if ($request = Requests::find($request_id)) {
                            if ($request->confirmed_walker == $walker_id) {
                                ...
                                if ($request->is_started == 1) {
                                    ...
                                    $request->time = 0;
                                    if ($request->is_started == 1) {
                                        //all the time value are override by here because of accuracy
                                        //trip start time is when first row of trip in walkerlocation table
                                        $time = WalkLocation::where('request_id', $request->id)->first();
                                        if($time)
                                        {
                                            $difference = isCheckDateDifferenceReturnMinutes($time->created_at);
                    ...
                    if (Input::get('new_password')) {
                        if (Input::get('old_password') != "") {
                            if (Hash::check($old_password, $walker_data->password)) {
                                ...
                                $response_array = array('success' => false, 'error' => 'Invalid Old Password', 'error_code' => 624);
                                $response_code = 200;
                            }
                        } else {
                ...
                if (is_token_active($walker_data->token_expiry) || $is_admin) {
                    if ($from != "" && $to_date != "") {
                        $request_data = DB::table('request')
                                ...
                                ->select('request.*', 'request.request_start_time', 'request.transfer_amount', 'owner.first_name', 'owner.last_name', 'owner.phone', 'owner.email', 'owner.picture', 'owner.bio', 'request.distance', 'request.time', 'request.promo_code', 'request_services.base_price', 'request_services.distance_cost', 'request_services.time_cost', 'request.total','request_services.service_tax')
                                ->groupBy('request.id')
                                ->get();
                    } else {
                        $request_data = DB::table('request')
                   ...
                   /* echo "<pre>";
                    ...
                    foreach ($request_data as $data) {
                        ...
                        if ($is_multiple_service->value == 0) {
                          ...
                          /*  if(empty($requestserv)){
                            ...
                            if ($setting_zone->value == 1) {
                                ...
                                $request['base_price'] = currency_converted(empty($zoneTypesRecs[0]->base_price) ? 0 : $zoneTypesRecs[0]->base_price);
                                $request['distance_price'] = currency_converted(empty($zoneTypesRecs[0]->price_per_unit_distance) ? 0 : $zoneTypesRecs[0]->price_per_unit_distance);
                                $request['price_per_unit_time'] = currency_converted(empty($zoneTypesRecs[0]->price_per_unit_time) ? 0 : $zoneTypesRecs[0]->price_per_unit_time) ;
                        ...
                        if(!empty($adminPercentage) && $adminPercentage != 0) {
                            ...
                            $request['admin_per_payment'] = $calcAdminPerPayment;  //Assign Percantage calcuation of admin
                            $request['driver_per_payment'] = $calcDriverPerPayment; //Assign Percantage calcuation of driver
                        }
...
// check for token validity
                    ...
                    if ($is_admin) {
                        $response_array = array('success' => false, 'error' => 'Walker ID not Found', 'error_code' => 621);
                    } else {
                        $response_array = array('success' => false, 'error' => 'Not a valid token', 'error_code' => 622);
                    }
                    $response_code = 200;
    ...
    public function get_walker_disable()
    {
        $minutes = 15;
        /*get login available Walker*/
        $query = "SELECT id,email,updated_at,TIMESTAMPDIFF(MINUTE,updated_at,CURRENT_TIMESTAMP) as minutes from walker WHERE is_active = 1 AND is_available = 1 AND is_approved = 1 AND TIMESTAMPDIFF(MINUTE,updated_at,CURRENT_TIMESTAMP) > $minutes";
        $walkerRec = DB::select(DB::raw($query));

        $walkerUnavailableUpdate = array();

        if (is_array($walkerRec) && !empty($walkerRec)) {

            foreach ($walkerRec as $key => $walker) {

                $walkerUnavailableUpdate[] = $walker->id;

            }
        }

        //Walker is not active above 15 Minutes
        DB::table('walker')
            ->whereIn('id', $walkerUnavailableUpdate)
            ->update(array('is_active' => 0));


    }

    public function isOwnerRegisterProcessChecking($phoneNumber) {

        if (!empty($phoneNumber)) {
            $ownerData = Owner::where('phone', '=', $phoneNumber)->first();

            if(empty($ownerData)) {

                $owner = new Owner;
                $owner->first_name = "stranger";
                $owner->last_name = "stranger";
                $owner->phone = $phoneNumber;
                $owner->login_by = "manual";
                $owner->email = RandomString(5)."_".RandomString(5)."@gmail.com";
                $owner->timezone = Config::get('app.timezone');
                $owner->save();

                regenerate:
                $referralCode = my_random4_number();
                if (Ledger::where('referral_code', $referralCode)->count()) {
                    goto regenerate;
                }

                /* Referral value store  Legder*/
                $ledger = new Ledger;
                $ledger->owner_id = $owner->id;
                $ledger->referral_code = $referralCode;
                $ledger->save();
    ...
    public  function isWalkerCreateNewRequest($ownerId,$walkerData,$zone){

        //Get the Walker Time Zone
        $walkerTimezone = $walkerData->timezone;
        $defaultTimezone = 'UTC';
        $dateTime = get_user_time($defaultTimezone, $walkerTimezone, date("Y-m-d H:i:s"));

        $request = new Requests;
        $request->owner_id = $ownerId;
        $request->confirmed_walker = $walkerData->id;
        $request->current_walker = $walkerData->id;
        $request->request_start_time = $dateTime;
        $request->is_walker_started = 1;
        $request->is_walker_arrived	 = 1;
        $request->payment_mode = 1;
        $request->request_type = 'Hand';
        $request->status = 1;
        $request->zone_id = $zone;
        $request->save();

        $reqService = new RequestServices;
        $reqService->request_id = $request->id;
        $reqService->type = $walkerData->type;
        $reqService->save();
    ...
    public function  create_request(){
        ...
        if ($validator->fails()) {
            ...
            if ($walkerData = $this->getWalkerData($walker_id, $token, $isAdmin)) {

                if ( is_token_active($walkerData->token_expiry) || $isAdmin ) {

                    if($walkerData->is_approved == 0 )
                    {
                        $responseArray = array('success' => false, 'error' => "Driver Can't create trip", 'error_code' => 653);
                        $responseCode = 200;
                        goto fail;
                    }
                    else if($walkerData->is_active == 0)
                    {
                        $responseArray = array('success' => false, 'error' => "Driver Can't create trip", 'error_code' => 654);
                        $responseCode = 200;
                        goto fail;
                    }
                    else if($walkerData->is_available == 0){

                        $responseArray = array('success' => false, 'error' => "Driver already in trip", 'error_code' => 655);
                        $responseCode = 200;
                        goto fail;
...
//die('c');
                    if(empty($phoneNumber)) {
                        $phoneNumber = "+00".RandomNumber(5);
                    }
                    //New User Register or Already existing user Checking
                    if( $ownerId = $this->isOwnerRegisterProcessChecking($phoneNumber) ) {
                        if( $request_id = $this->isWalkerCreateNewRequest($ownerId,$walkerData,$result['id']) ) {


                            $requests['id'] = $request_id;
                            $walkerData->is_available = 0;
                            //$walkerData->zone_id = $result['id'];
                ...
                if ($isAdmin) {
                    ...
                    $responseArray = array('success' => false, 'error' => Config::get('app.errorKeywords.provider_id_not_found'), 'error_code' => 410);
    ...
    public function  create_request_old(){
            ...
            if ($walkerData = $this->getWalkerData($walker_id, $token, $isAdmin)) {
                ...
                if ( is_token_active($walkerData->token_expiry) || $isAdmin ) {
                    ...
                    if( $ownerId = $this->isOwnerRegisterProcessChecking($phoneNumber) ) {
                        ...
                        if( $request_id = $this->isWalkerCreateNewRequest($ownerId,$walkerData) ) {
    ...
    public function  walk_save(){

        $reason = Input::get('reason');

        $request = new ExtraPayment;
        $request->request_id = 1;
        $request->reaseon = $reason;
    ...
    public function  walk_list(){

        $id = Input::get('id');

        $users = ExtraPayment::where('id', '=', $id)->first();
    ...
    public function get_item()
    {
        ...
        $columNames = Input::get('columNames');
        $whereName = Input::get('whereName');
        $whereId = Input::get('whereId');
        $expColumName = explode(",",$columNames);

        $rules = array(
            'table' => 'required',
            'columNames' => 'required',
            'whereName' => 'required',
            'whereId' => 'required|numeric',
        );
        ...
        if ($validator->fails()) {
            ...
            $errorMessages = $validator->messages()->all();
            $responseArray = array('success' => false, 'error' => 'Input invalid', 'error_code' => 401, 'error_messages' => $errorMessages);
            $responseCode = 200;

        } else {

            $users = DB::table($table)->select($expColumName)
                ->where($whereName, '=', $whereId)
                ->get();
    ...
    public function  providerCompletedTripHistory(){

        $token = Input::get('token');
        $walker_id = Input::get('id');
        $fromDate = Input::get('from_date'); // 2017-03-21
        $toDate = Input::get('to_date'); //2017-03-11
        ...
        if ($validator->fails()) {
            ...
            if ($walkerData = $this->getWalkerData($walker_id, $token, $isAdmin)) {
                ...
                if ( is_token_active($walkerData->token_expiry) || $isAdmin ) {
                    ...
                    $fromDate = ( !empty( $fromDate ) ? $fromDate : date("Y-m-d", strtotime("7 days ago")) );
                    $toDate = ( !empty( $toDate ) ? $toDate : date("Y-m-d") );

                    $dateDiff = dateDiff($fromDate, $toDate);
                    ...
                    if($dateDiff == 0) {
                        ...
                        $requestDatas = DB::table('request')
                            ...
                            ->select('request.request_start_time', 'request.is_completed', 'request.is_paid', 'request.driver_per_payment', 'request.total')
                            ->groupBy('request.id')
                            ->get();
                        ...
                        $requestDatas = DB::table('request')
                            ...
                            ->where('request.is_completed', 1)
                            ->where('request.is_paid', 1)
                            ->where('request_start_time', '>=', $fromDate)
                            ->where('request_start_time', '<=', $toDate)
                            ->Join('walker', 'request.confirmed_walker', '=', 'walker.id')
                            ->select('request.request_start_time', 'request.is_completed', 'request.is_paid', 'request.driver_per_payment', 'request.total')
                            ->groupBy('request.id')
                            ->get();
                    ...
                    if(count($requestDatas) > 0) {

                        $requests = array();
                        $total = $driverPerPayment = $totalTrip = 0;
                        $i = 1;
                        foreach($requestDatas as $key => $request) {

                            $totalTrip = $i;
                            $total += $request->total;
                            $driverPerPayment += $request->driver_per_payment;
                            $i++;
                        }
                        $requests['currency'] = Config::get('app.generic_keywords.Currency');
                        $requests['walker_id'] = $walker_id;
                        $requests['total'] = $total;
                        $requests['total_trip'] = $totalTrip;
                        $requests['driver_per_payment'] = $driverPerPayment;

                        $responseCode = 200;
                        $responseArray = array( 'success' => true, 'request' => $requests );

                    } else {

                        $responseCode = 200;
                        $responseArray = array( 'success' => false, 'error' => 'No Records Found', 'error_code' => 999 );
     ...
     public function  providerAccountLists(){
        ...
        if ($validator->fails()) {
            ...
            if ($walkerData = $this->getWalkerData($walkerId, $token, $isAdmin)) {

                if ( is_token_active($walkerData->token_expiry) || $isAdmin ) {

                    $bankAccounts = DB::table('walker_banking_details as wb')
                        ->select('wb.*')
                        ->where('wb.walker_id','=', $walkerId)
                    ...
                    if(count($bankAccounts) > 0 ) {
                        foreach ($bankAccounts as $bankAccount) {
                            ...
                            $data['walker_id'] = $bankAccount->walker_id;
                            $data['merchant_id'] = $bankAccount->merchant_id;
                            $data['dob'] = date("d-m-Y h:i:s", strtotime($bankAccount->dob));
                            $data['ssn'] = $bankAccount->ssn;
                            $data['account_number'] = $bankAccount->account_number;
                            $data['routing_number'] = $bankAccount->routing_number;
                            $data['is_default'] = (!empty($bankAccount->is_default) ? true : false);
                    ...
                    $responseArray = array(
                        'success' => true,
                        'bankAccounts' => ( !empty( $records ) ? $records : null)
                    );
    ...
    public function  providerSpendAmounts(){
        ...
        if ($validator->fails()) {
            ...
            if ($walkerData = $this->getWalkerData($walkerId, $token, $isAdmin)) {
                ...
                if ( is_token_active($walkerData->token_expiry) || $isAdmin ) {
                    ...
                    //$result = Braintree_PaymentMethodNonce::create($clientToken);
                    //$nonce = $result->paymentMethodNonce->nonce;


                    if(!$bank_detail=DB::table('walker_banking_details')->where('walker_id' , $walkerId)->where('is_default' , 1)->first())
                    {
                        $responseArray = array('success' => false, 'error' => 'No Account found', 'error_code' => 405);
                    ...
                    $result = Braintree_Transaction::sale([
                        'amount' => $amount,
                        'merchantAccountId' => $bank_detail->merchant_id,
                    ...
                    if ($result->success) {
                        $walkerTotAmt = $walkerSpendAmt = 0;
                        $walkerSpendHistory = new WalkerSpendHistory();
                        $walkerSpendHistory->spend_history_id = $spendHistoryId = SpendingUniqueIdGeneration();
                        $walkerSpendHistory->walker_id = $walkerId;
                        $walkerSpendHistory->amount = $amount;
                        $walkerSpendHistory->save();

                        $walletSpendTotal = DB::table('walker_wallet_state')->where('walker_id', '=', $walkerId)->sum('total_spend');
                        $walletTotalAmt = DB::table('walker_wallet_state')->where('walker_id', '=', $walkerId)->sum('total_balance');

                        if (!empty($walletTotalAmt)) {

                            $walkerSpendAmt = $walletSpendTotal + $amount;
                            $walkerTotAmt = $walletTotalAmt - $amount;

                            DB::table('walker_wallet_state')->where('walker_id', '=', $walkerId)
                                ->update(
                                    array(
                                        'total_spend' => $walkerSpendAmt,
                                        'total_balance' => $walkerTotAmt
                                    )
                                );
                        }

                        $records = array();
                        $records['walker_id'] = $walkerId;
                        $records['spend_history_id'] = $spendHistoryId;
                        $records['total_balance'] = $walkerTotAmt;
                        $records['total_spend'] = $walkerSpendAmt;
                        $records['amount'] = $amount;
                    } else {
                        $records = array();
                        $records['error'] = "Something went wrong";
                    }
                    //echo "<pre>";
                    //print_r($records); exit;
                    ...
                    $responseArray = array(
                        'success' => true,
                        'walker_spend_amount' => ( !empty( $records ) ? $records : null)
                    );
        ...
        if ($validator->fails()) {
            ...
            if ($walkerData = $this->getWalkerData($walkerId, $token, $isAdmin)) {
                ...
                if ( is_token_active($walkerData->token_expiry) || $isAdmin ) {
                    ...
                    $walker_amount=WalkerWalletState::where('walker_id', $walkerId)->first();
                    if($walker_amount)
                    {
                        $responseArray = array('success' => true,
                            'wallet' => array(
                                'total_amount' => $walker_amount->total_amount,
                                'total_balance' => $walker_amount->total_balance,
                                'total_spend' => $walker_amount->total_spend,
                                ));
                    }else
                    {
                        $responseArray = array('success' => true,
