            ...
            if ($owner_data = $this->getOwnerData($owner_id, $token, $is_admin)) {
                ...
                if (is_token_active($owner_data->token_expiry) || $is_admin) {
                    ...
                    /*  foreach ($walkers as $key) {
                          ...
                          $provider[$p]['current'] = array('latitude' => round($key->latitude, 7), 'longitude' => round($key->longitude, 7));

                          $provider[$p]['bearing'] = round($key->bearing, 7);

                          $provider[$p]['type'] = intval($key->type);

                          $provider[$p]['is_near'] = $near;
                          $p++;
                        ...
                        if (Input::has('type')) {
                            ...
                            if ($setting_zone->value == 1) {
                                ...
                                if (!empty($zoneRecords)) {
                                    ...
                                    $result = isCheckZoneDivisionFound($zoneRecords, $latitude, $longitude);
                                }
