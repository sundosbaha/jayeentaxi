...
class Apns_W {
    ...
    public function initialize_apns() {
        try {
            ...
            //echo $this->getCertificatePath(); exit;
    ...
    public function send_notification($devices, $message) {
        try {
			...
			echo 'Result :- '.$result;
            echo "<br/>";
            echo "bodyError".$bodyError;
            echo "<br/>";
            echo "errCounter".$errCounter;
            exit; */
            if ($result) {
                Log::info('Delivered Message to APNS' . ''.$bodyError . PHP_EOL);
