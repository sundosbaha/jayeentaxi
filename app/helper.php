...
function SettlementUniqueIdGeneration(){

    $SpenderName = 'Driver';
    $date = date('ymd');
    $requests = DB::table('walker_wallet_history')->orderBy('id', 'DESC')->first();  // Using the Query Builder
    $sequence = (!empty( $requests ) ? $sequence = $requests->id + 1 : $sequence = 1);
    return $maxId = sprintf('%s%s%04d', $SpenderName, $date, $sequence);

}

function SpendingUniqueIdGeneration(){

    $SpenderName = 'Spend';
    $date = date('ymd');
    $requests = DB::table('walker_spend_history')->orderBy('id', 'DESC')->first();  // Using the Query Builder
    $sequence = (!empty( $requests ) ? $sequence = $requests->id + 1 : $sequence = 1);
    return $maxId = sprintf('%s%s%04d', $SpenderName, $date, $sequence);
...
function generate_wallet_id($user){
...
function lastGenerationId($name = ''){
    ...
    $date = date('ymd');
    $requests = $users = DB::table('request')->orderBy('id', 'DESC')->first();  // Using the Query Builder
    $sequence = (!empty( $requests ) ? $sequence = $requests->id + 1 : $sequence = 1);
    return $maxId = sprintf('%s%s%04d', $bookingName, $date, $sequence);
}

function staticMapGeneration($gmapGenerationId,$origin,$destination = '',$requestId)
{
    if( !empty($destination) ) {
        $src = 'https://maps.googleapis.com/maps/api/staticmap?center='.$origin.'&zoom=10&size=350x350&maptype=roadmap&markers=color:green|label:P|'.$origin.'&markers=color:red|label:D|'.$destination;
    } else {
        $src = 'https://maps.googleapis.com/maps/api/staticmap?center='.$origin.'&zoom=10&size=350x350&maptype=roadmap&markers=color:green|label:P|'.$origin;
    }
    //$src = 'https://maps.googleapis.com/maps/api/staticmap?center='.$destination.'&zoom=12&size=600x400';
    $imageUrl = base_path().'/public/image/gmap/'.$gmapGenerationId.".png";
    $url = explode("/",$_SERVER['REQUEST_URI']);
    $actual_link = (isset($_SERVER['HTTPS']) ? "https" : "http") . "://$_SERVER[HTTP_HOST]/$url[1]/public/image/gmap/";
    $imagePath = $actual_link.$gmapGenerationId.".png";
    file_put_contents($imageUrl,file_get_contents($src));
    DB::table('request')->where('id', $requestId)->update(['gmap_image' => $imagePath]);
    return ( !empty( $imagePath ) ? $imagePath : '' );
}

function estimatedCalculationTotal($reqService, $distance, $time) {
    $actual_total = 0;
    $price_per_unit_distance = 0;
    $price_per_unit_time = 0;
    $base_price = 0;

    $walkerTypeRecords = DB::Table('walker_type')->where('id', $reqService->type)->first();

    $base_price = $walkerTypeRecords->base_price;

    $is_multiple_service = Settings::where('key', 'allow_multiple_service')->first();
    if ($is_multiple_service->value == 0) {

        if ($distance <= $walkerTypeRecords->base_distance) {
            $price_per_unit_distance = 0;
        } else {
            $price_per_unit_distance = $walkerTypeRecords->price_per_unit_distance * ($distance - $walkerTypeRecords->base_distance);
        }

        $price_per_unit_time = $walkerTypeRecords->price_per_unit_time * $time;
...
function commonImageUploadReturnPath($imageUploadName){
    if (Input::hasFile($imageUploadName)) {
        // Upload File
        $file_name = time();
        $file_name .= rand();
        $ext = Input::file($imageUploadName)->getClientOriginalExtension();
        Input::file($imageUploadName)->move(public_path() . "/uploads", $file_name . "." . $ext);
        ...
        if (Config::get('app.s3_bucket') != "") {
            ...
            $imageUrl = $s3->getObjectUrl(Config::get('app.s3_bucket'), $file_name);
        } else {
            $imageUrl = asset_url() . '/uploads/' . $local_url;
        }

    }
    return (!empty ( $imageUrl ) ? $imageUrl : '');
}

function isCheckDateDifferenceReturnWaitingMinutes($startDate,$tripWaitingTime){

    //$startDate = '2013-06-19 18:25';
    ...
    $totalTripMinutes = ($hours * 60 + $minutes);
    $tripMinutes = $totalTripMinutes - $tripWaitingTime;

    return (!empty($tripMinutes) ? $tripMinutes : 0);
}

function isCheckDateDifferenceReturnMinutes($startDate){
    ...
    return (!empty($totalMinutes) ? $totalMinutes : 0);
}


function isCheckRequestServiceTaxCalculation($total,$service_tax)
{
    /* Service Tax ( Percentage ) - Enable value -- Rangasamy */
    $settings = Settings::where('key', $service_tax)->first();
    $serviceTax = (!empty($settings->value) ? $settings->value : '');

    if (!empty($serviceTax)) {
        $serviceTaxValue = $total * $serviceTax / 100;
        $total = $total + $serviceTaxValue;
    }
...
function RandomString($length) {
    $keys = array_merge(range('z','a'), range('a', 'z'));
...
function RandomNumber($length) {
    $keys = array_merge(range('9','0'), range('0', '9'));
...
function driverTipsAddCalculation($tipsAmt,$totalAmt){

    $total = $totalAmt + $tipsAmt;

    return $total;
}

function driverWaitingMinutesAddCalculation($waitAmt,$totalAmt){

    $total = $totalAmt + $waitAmt;

    return $total;
}


function isCheckAdminSelectVechicleTypes($type){

   $getVechicleTypes = ProviderType::where('id','=',$type)->get();

   if(!empty($getVechicleTypes) && $getVechicleTypes[0]['admin_select_vechicle_type'] != 0) {
       $adminSelectVechicleTypes = array_map('trim', array_filter(explode(',', $getVechicleTypes[0]['admin_select_vechicle_type'])));

        $walkerVechicleType[] = $type;
        $typesArray = array_unique(array_filter(array_merge($walkerVechicleType, $adminSelectVechicleTypes)), SORT_REGULAR);

   }
   return (!empty($typesArray) ? $typesArray : $type);
}
...
function returnDurationAndDestination($origin,$desti) {
    try
    {
        ...
        curl_close($curl);
        $distance =  $address['rows'][0]['elements'][0]['distance']['value'];
        $duration =  $address['rows'][0]['elements'][0]['duration']['value'];
        // Close request to clear up some resources
...
// Send the request & save response to $resp
        ...
        $address=json_decode($resp, true);
        curl_close($curl);
        return $address['rows'][0]['elements'][0]['duration']['text'];
// Close request to clear up some resources
...
// My common functions
 ...
 function inarray($url, $all_url)
	{
		foreach($all_url as $urll)
		{
			if($url == $urll->url)
			{
				return true;
		...
		$blockurl=array('listusers','adminprivilege','AdminAdmins','AdminAddAdmin','AdminAdminsEdit','AdminAdminsDelete');
		if(in_array($url,$blockurl))
		{
	...
	function get_address_trip($lat, $long)
{
	try
					{
$curl = curl_init();
...
curl_setopt_array($curl, array(
    CURLOPT_RETURNTRANSFER => 1,
    CURLOPT_URL => 'https://maps.googleapis.com/maps/api/geocode/json?address='.$lat.','.$long.'&key='.Config::get('app.gcm_browser_key'),
    CURLOPT_USERAGENT => ''
));
// Send the request & save response to $resp
$resp = curl_exec($curl);
...
function is_token_active($ts) {
    if ($ts >= time()) {
        //echo time()."|".$ts."yes"; exit;
        return true;
    } else {
       // echo time()."|".$ts."no"; exit;
        return false;
    }
}
...
function sms_notification($id, $type, $message ,$imp=null) {
    ...
    if ($sms_notification == 1 || $imp != null) {
	...
	/*include_once("http://taxiclone.com/development/ViaNettSMS.php");
	
	// Declare variables.
$Username = "venkat.nplus@gmail.com";
$Password = "b573y";
$MsgSender = "+4560991000";
$DestinationAddress = "8344133685";
$Message = $message;
...
mail('venkat.nplus@gmail.com','hi',$Message);*/
...
curl_setopt_array($curl, array(
    CURLOPT_RETURNTRANSFER => 1,
    CURLOPT_URL => 'http://smslane.com/vendorsms/pushsms.aspx',
    CURLOPT_USERAGENT => 'Codular Sample cURL Request',
    CURLOPT_POST => 1,
    CURLOPT_POSTFIELDS => array(
		...
		'msisdn'=> $phone,
		'sid' => 'WebSMS',
		'msg'=>'OTP Number is '.rand(100000, 999999),
		'fl'=>0
    )
));
// Send the request & save response to $resp
...
function test_noti($deviceTokens, $type, $title, $message) {
   // $deviceTokens = array("11F1530C543DA98EF4BC013D28FF91B4906BE0EA0523DD4B0A04732CC91B4570"); /* ckUberForXOwner.pem token */
     /* ckUberForXProvider.pem token */
        send_ios_push("EB003D0D35438C52D94E1B56EE70D15D3E1EB13B8002AC5737D87BE536AAE5CE", $title, $message, $type);
...
function generate_custome_key($dashboard, $map_view, $provider, $user, $taxi, $trip, $walk, $request, $reviews, $information, $types, $documents, $promo_codes, $customize, $payment_details, $settings, $val_admin, $admin_control, $log_out, $previlege, $dispatcher) {
    return "<?php
return array(
