...
?>
<a href="{{ URL::Route('AdminSettingInstallation') }}">
    <button class="btn btn-flat btn-block btn-success" id="newinstall">Installation Settings</button>
</a>
...
<div class="row">
                            ...
                            if ($setting->key != 'default_distance_unit' && $setting->key != 'sms_notification' && $setting->key != 'zone_division' && $setting->key != 'email_notification' && $setting->key != 'push_notification' && $setting->key != 'default_charging_method_for_users' && $setting->key != 'referral_code_activation' && $setting->key != 'promotional_code_activation' && $setting->key != 'get_referral_profit_on_card_payment' && $setting->key != 'get_referral_profit_on_cash_payment' && $setting->key != 'get_promotional_profit_on_card_payment' && $setting->key != 'get_promotional_profit_on_cash_payment' && $setting->key != 'otp_verification') {
                                if ($setting->key == 'base_price' || $setting->key == 'price_per_unit_distance' || $setting->key == 'price_per_unit_time') {
                                    ...
                                    <div class="form-group">
                                        <label><?php echo ucwords(str_replace("_", " ", $setting->key)); ?>&nbsp;<a
                                                href="#" data-toggle="tooltip" title="<?= $setting->tool_tip; ?>"><img
                                                    src="<?php echo asset_url(); ?>/image/icon-tooltip.jpg"></a></label>

                                        <?php if ((strstr($setting->key, 'email_') || strstr($setting->key, 'sms_')) & $setting->key != 'admin_email_address') { ?>
                                            <textarea style="resize: none;" class="form-control" rows="2" cols="50"
                                                      name="<?php echo $setting->id; ?>"><?php echo $setting->value; ?></textarea>
                                        <?php } else { ?>
                                            <?php if ($setting->key == 'base_price' || $setting->key == 'price_per_unit_distance' || $setting->key == 'price_per_unit_time' || $setting->key == 'default_referral_bonus' || $setting->key == 'default_referral_bonus_to_refered_user' || $setting->key == 'default_referral_bonus_to_refereel') { ?>
                                                <span id="no_amount_error<?php echo $counter; ?>"
                                                      style="display: none"></span>
                                                <input class="form-control" type="text"
                                                       name="<?php echo $setting->id; ?>"
                                                       value="<?php echo $setting->value; ?>"
                                                       onkeypress="return Isamount(event,<?php echo $counter; ?>);">
                                                <?php
                                            } else if ($setting->key == 'provider_timeout') {
                                                ?>
                                                <span id="no_number_error<?php echo $counter; ?>"
                                                      style="display: none"> </span>
                                                <input class="form-control" type="text"
                                                       name="<?php echo $setting->id; ?>"
                                                       value="<?php echo $setting->value; ?>"
                                                       onkeypress="return IsNumeric(event,<?php echo $counter; ?>);">
                                                <?php
                                            } else if ($setting->key == 'default_search_radius' || $setting->key == 'map_center_latitude' || $setting->key == 'map_center_longitude') {
                                                ?>
                                                <span id="no_numamount_error<?php echo $counter; ?>"
                                                      style="display: none"></span>
                                                <input class="form-control" type="text"
                                                       name="<?php echo $setting->id; ?>"
                                                       value="<?php echo $setting->value; ?>"
                                                       onkeypress="return Isnumamount(event,<?php echo $counter; ?>);">
                                            <?php } else if ($setting->key == 'admin_phone_number') { ?>
                                                <span id="no_mobile_error<?php echo $counter; ?>"
                                                      style="display: none"> </span>
                                                <input class="form-control" type="text"
                                                       name="<?php echo $setting->id; ?>"
                                                       value="<?php echo $setting->value; ?>"
                                                       onkeypress="return Ismobile(event,<?php echo $counter; ?>);">
                                            <?php } else if ($setting->key == 'admin_email_address') {
                                                ?>
                                                <span id="no_email_error<?php echo $counter; ?>"
                                                      style="display: none"> </span>
                                                <input class="form-control" type="text"
                                                       name="<?php echo $setting->id; ?>"
                                                       value="<?php echo $setting->value; ?>"
                                                       onblur="ValidateEmail(<?php echo $counter; ?>)"
                                                       id="email_check<?php echo $counter; ?>" required="">
                                            <?php } else {
                                                ?>
                                                <input class="form-control" type="text"
                                                       name="<?php echo $setting->id; ?>"
                                ...
                                if ($setting->key != 'default_charging_method_for_users') {
                                    ...
                                    <div class="form-group">
                                        ...
                                        <select <?php if ($setting->key == 'otp_verification') { ?> id="otp" onChange="otpchange('<?php if ($install['twillo_account_sid'] == '' && $install['twillo_auth_token'] == '' && $install['twillo_number'] == '') {
                                            echo 'false';
                                        } else {
                                            echo 'true';
                                        } ?>')" <?php } ?> class="form-control" name="<?php echo $setting->id; ?>">
                                            <option value="1" <?php
                                            if ($setting->value == 1) {
                                                echo "selected";
                                            }
                ...
                foreach ($settings as $setting) {
                    if ($setting->page == 2) {
                        if ($setting->key != 'sms_notification' && $setting->key != 'email_notification' && $setting->key != 'push_notification' && $setting->key != 'default_charging_method_for_users') {
                            ?>
                            <div class="form-group">
                                <label><?php echo ucwords(str_replace("_", " ", $setting->key)); ?>&nbsp;<a href="#"
                                                                                                            data-toggle="tooltip"
                                                                                                            title="<?= $setting->tool_tip; ?>"><img
                                            src="<?php echo asset_url(); ?>/image/icon-tooltip.jpg"></a></label>

                                <?php if ((strstr($setting->key, 'email_') || strstr($setting->key, 'sms_')) & $setting->key != 'admin_email_address') { ?>
                                    <textarea style="resize: none;" class="form-control" rows="1" cols="50"
                                              name="<?php echo $setting->id; ?>"><?php echo $setting->value; ?></textarea>
                                <?php } else { ?>
                                    <input class="form-control" type="text" name="<?php echo $setting->id; ?>"
                                           value="<?php echo $setting->value; ?>">
                                <?php } ?>
            ...
            <div class="box-body">
                ...
                foreach ($settings as $setting) {
                    if ($setting->page == 4) {
                        ...
                        } else if ($setting->key == 'pre_auth_amt') {
                            if (Config::get('app.generic_keywords.Currency') == '$') {
                                ?>
                                <div class="form-group">
                                    <label><?php echo ucwords(str_replace("_", " ", $setting->key)); ?>&nbsp;<a href="#"
                    ...
                    <?php if ($setting->key == 'get_destination') { ?>
                        <div class="form-group">
                            ...
                            <p class="help-block">Note: If Destination is disabled, Share ETA and Fare Calculation will
                                not work.</p>
                        ...
                        if ($setting->key != 'paypal' && $setting->key != 'promo_code') {
                            ...
                            <div class="form-group">
                                <?php if ($setting->key == 'cod') { ?>
                                    <label><?php echo ucwords("Cash payment option"); /* ucwords(str_replace("_", " ", $setting->key)); */ ?>
                                        &nbsp;<a href="#" data-toggle="tooltip" title="<?= $setting->tool_tip; ?>"><img
                                                src="<?php echo asset_url(); ?>/image/icon-tooltip.jpg"></a></label>
                                <?php } else { ?>
                                    <label><?php echo ucwords(str_replace("_", " ", $setting->key)); ?>&nbsp;<a href="#"
                                                                                                                data-toggle="tooltip"
    ...
    <div class="col-md-6 col-sm-12">
        <div class="box box-primary">
            ...
            <!-- form start -->
            <form enctype="multipart/form-data" onsubmit="Checkfiles(this)" method="post"
                  action="{{ URL::Route('AdminTheme') }}">
                <div class="box-body">
                    <div class="form-group">
                        <label>Logo</label>
                    ...
                    <div class="form-group">
                        ...
                        <?php if (isset($theme->id)) { ?>
                            <img src="<?php echo asset_url(); ?>/uploads/<?= $theme->favicon; ?>" height="40"
                                 width="40">
                            <br>
                        <?php } ?>
                    </div>
...
<script type="text/javascript">
    function otpchange(test) {
        if (test == 'false') {
            alert('Please configure sms gateway first');
            $('#otp').val(0);
        }
    }
