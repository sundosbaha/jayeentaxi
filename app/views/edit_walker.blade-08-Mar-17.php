    ...
    <form method="post" id="main-form" action="{{ URL::Route('AdminProviderUpdate') }}" enctype="multipart/form-data">
        ...
        <div class="box-body">
            <div class="form-group">
                ...
                <input type="text" class="form-control" name="first_name" value="<?= $walker->first_name ?>"
                       placeholder="First Name" _disabled>
            </div>

            <div class="form-group">
                <label>Last Name</label>
                <input class="form-control" type="text" name="last_name" value="<?= $walker->last_name ?>"
                       placeholder="Last Name" _disabled>
            </div>

            <div class="form-group">
                <label>Email</label>
                <input class="form-control" type="email" name="email" value="<?php echo $walker->email ?>"
                       placeholder="Email" readonly>
            </div>

            <div class="form-group">
                <label>Phone</label>
                <input class="form-control" type="text" name="phone" value="<?php echo $walker->phone ?>"
            ...
            <div class="form-group">
                <label>Address</label>
                <input class="form-control" type="text" name="address" value="<?= $walker->address ?>"
                       placeholder="Address" _disabled>
            </div>
            ...
            <div class="form-group">
                <label>Country</label>
                <input class="form-control" type="text" name="country" value="<?= $walker->country ?>"
                       placeholder="Country" _disabled>
            </div>

            <div class="form-group">
                <label>Zip Code</label>
                <input class="form-control" type="text" name="zipcode" value="<?= $walker->zipcode ?>"
                       placeholder="Zip Code" _disabled>
            </div>

            <div class="form-group">
                <label>Car Number</label>
                <input class="form-control" type="text" name="car_number" value="<?= $walker->car_number ?>"
                       placeholder="Zip Code" _disabled>
            </div>

            <div class="form-group">
                <label>Car Model</label>
                <input class="form-control" type="text" name="car_model" value="<?= $walker->car_model ?>"
                       placeholder="Zip Code" _disabled>
            </div>

            <div class="form-group">
                <label><?php echo Lang::get('customize.zone_name'); ?></label>
                <select name="zone_id" class="form-control" id="zone_id">
                    <option value=''>--Select Zone Name--</option>
                    <?php foreach ($zoneTypes as $zoneType) :
                        if ($zoneType->id == $walker->zone_id) :
                            $selected = "selected"; ?>
                            <option value="{{$zoneType->id}}" <?php echo $selected; ?>>{{$zoneType->zone_name}}</option>
                        <?php else : ?>
                            <option value="{{$zoneType->id}}">{{$zoneType->zone_name}}</option>
                        <?php endif;
                    endforeach; ?>
                </select>
            </div>
                ...
                <table class="table table-bordered">
                    ...
                    <tr>
                        ...
                        <td>
                            ...
                            <input class="form-control" name="service_base_price[{{$types->id}}]" type="text"
                                   onkeypress="return Isamount(event,<?php echo $counter; ?>);" value="<?php
                        ...
                        <td>
                            <?php $counter++; ?>
                            <span id="no_amount_error<?php echo $counter; ?>" style="display: none"></span>
                            <input class="form-control" name="service_price_distance[{{$types->id}}]" type="text"
                                   onkeypress="return Isamount(event,<?php echo $counter; ?>);" value="<?php
                            $proviserv = ProviderServices::where('provider_id', $walker->id)->where('type', $types->id)->first();
                            if (empty($proviserv)) {
                                echo "";
                        ...
                        <td>
                            <?php $counter++; ?>
                            <span id="no_amount_error<?php echo $counter; ?>" style="display: none"></span>
                            <input class="form-control" name="service_price_time[{{$types->id}}]" type="text"
                                   onkeypress="return Isamount(event,<?php echo $counter; ?>);" value="<?php
                            $proviserv = ProviderServices::where('provider_id', $walker->id)->where('type', $types->id)->first();
                            if (empty($proviserv)) {
                                echo "";
