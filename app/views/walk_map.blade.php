...
<div class="box box-info tbl-box" dir="{{ trans('language_changer.text_format') }}">
    <table class="table table-bordered">
        <tbody>
            <tr>
                ...
                <th>{{ trans('language_changer.Provider'),' ', trans('language_changer.name') }} </th>
                ...
                <th>{{ trans('language_changer.response')}}</th>
            </tr>
            <?php $i = 0; ?>
    ...
    <?php //$i++; ?>
                     ...
                     <td> <?php echo date('Y-m-d/H:m:s',strtotime($start_time)); ?> </td>
                    <td><?= $status1 ?></td>
                    ...
                    <td>
                        <?php
                        if ($meta->status == 0) {
                            echo "<span class='badge bg-yellow'>".trans('language_changer.in_queue')."</span>";
                        } elseif ($meta->status == 1) {
                            echo "<span class='badge bg-green'>".trans('language_changer.accepted')."</span>";
                        } elseif ($meta->status == 3) {
                            echo "<span class='badge bg-red'>".trans('language_changer.rejected')."</span>";
                        } else {
                            echo "<span class='badge bg-red'>".trans('language_changer.no')." ".trans('language_changer.response')."</span>";
                        }
                        ?>
                    </td>
                </tr>
...
<script src="https://maps.googleapis.com/maps/api/js?v=3.exp&key=<?= Config::get('app.gcm_browser_key'); ?>" type="text/javascript"></script>
...
controlDiv.style.padding = '5px';
        ...
        // Add the text
        controlText.innerHTML = '<b><?= trans('language_changer.legends') ?></b><br />' +
        '<img src="<?php echo asset_url(); ?>/image/start_pin_flag.png" style="height:25px;"/> <?= trans('language_changer.User') .trans('language_changer.start') ?><br />' +
        '<img src="<?php echo asset_url(); ?>/image/end_pin_flag.png" style="height:25px;"/> <?= trans('language_changer.Provider') . trans('language_changer.end') ?><br />' +
